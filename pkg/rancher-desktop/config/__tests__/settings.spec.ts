...
describe('settings', () => {
  ...
  describe('migrations', () => {
    it('empty settings end up with only a current version field', () => {
      const s: RecursivePartial<settings.Settings> = {};
      const result = settingsImpl.migrateSpecifiedSettingsToCurrentVersion(s);

      expect(result).toEqual({ version: settings.CURRENT_SETTINGS_VERSION });
    });

    it('typical setting gets a current version field', () => {
      expect(settingsImpl.migrateSpecifiedSettingsToCurrentVersion({ containerEngine: { name: settings.ContainerEngine.MOBY } })).toEqual(
        { version: settings.CURRENT_SETTINGS_VERSION, containerEngine: { name: settings.ContainerEngine.MOBY } });
    });
    ...
    it('unversioned no-proxy settings are correctly migrated', () => {
      const s: RecursivePartial<settings.Settings> = {
        experimental: {
          virtualMachine: {
            proxy: {
    ...
    it('unrecognized settings are left unchanged', () => {
      ...
      const expected = _.merge({ version: settings.CURRENT_SETTINGS_VERSION }, s);
    ...
    it('recorrects socket vmnet back to where it started', () => {
      const s: Record<string, any> = {
        version:        5,
        virtualMachine: {
          experimental: {
